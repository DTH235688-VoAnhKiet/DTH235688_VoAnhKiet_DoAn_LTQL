...
namespace QLDuAnPhanMemTinHoc.form
{
    public partial class Bug : Form
    {
        ...
        private void XayDungGiaoDien()
        {
            ...
            // NÚT XÓA BUG
            Button btnXoaBug = new Button
            {
                Text = "🗑️ XÓA BUG",
                Location = new Point(540, 148), // Nằm ngay cạnh nút Thêm Bug
                Size = new Size(120, 30),
                BackColor = Color.FromArgb(231, 76, 60), // Màu đỏ cảnh báo
                ForeColor = Color.White,
                FlatStyle = FlatStyle.Flat,
                Font = new Font("Segoe UI", 9, FontStyle.Bold),
                Cursor = Cursors.Hand
            };
            btnXoaBug.Click += BtnXoaBug_Click;
            this.Controls.Add(btnXoaBug);
        ...
        // --- HÀM MỚI PHỤC VỤ CHỨC NĂNG XÓA BUG ---
        private void BtnXoaBug_Click(object sender, EventArgs e)
        {
            if (maBugDangChon == -1)
            {
            ...
            if (hoiNhe == DialogResult.Yes)
            {
                using (SqlConnection conn = new SqlConnection(strKetNoi))
                {
                    ...
                    {
                        conn.Open();
                        string sql = "DELETE FROM Bug WHERE ID = @id";
                        SqlCommand cmd = new SqlCommand(sql, conn);
                        cmd.Parameters.AddWithValue("@id", maBugDangChon);
                        cmd.ExecuteNonQuery();

                        MessageBox.Show("Đã phi tang cái bug này thành công!", "Thông báo");

                        maBugDangChon = -1; // Reset lại mã bug sau khi xóa
        ...
        // --- CÁC HÀM PHỤC VỤ CHỨC NĂNG THÊM BUG ---
